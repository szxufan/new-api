...
// 空回复命中（TreatEmptyAsHit）需要调用 CheckResponseDetectionWithEmpty。
...
//   - 当 detection.TreatEmptyAsHit 为 true 且文本 trim 后为空且无工具调用时，返回 (true, nil)
...
func CheckResponseDetectionWithEmpty(text string, hasToolCalls bool, detection *dto.ResponseDetection) (bool, []string) {
	...
	if detection.TreatEmptyAsHit && !hasToolCalls && len(strings.TrimSpace(text)) == 0 {
		return true, nil
	}
