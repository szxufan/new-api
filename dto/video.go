...
type VideoRequest struct {
	...
	Images         []string       `json:"images,omitempty" example:"https://example.com/a.png"`                                                                                                                  // Multi-image input; mode inferred by count (1=first frame, 2=first-last, >=3=references; channel-specific, see docs/video-api.md)
	InputReference string         `json:"input_reference,omitempty" example:"https://example.com/ref.png"`                                                                                                       // Reference media (OpenAI-compatible), treated as first frame
	Size           string         `json:"size,omitempty" example:"1280x720"`                                                                                                                                     // Resolution / dimensions (channel-specific)
	Duration       float64        `json:"duration" example:"5.0"`                                                                                                                                                // Video duration (seconds)
	Seconds        string         `json:"seconds,omitempty" example:"5"`                                                                                                                                         // Duration as string (used by some channels, e.g. doubao)
	Mode           string         `json:"mode,omitempty" example:"std"`                                                                                                                                          // Kling quality tier (std/pro); NOT the generation mode
	...
	Metadata       map[string]any `json:"metadata,omitempty"`                                                                                                                                                    // Vendor-specific/custom params; supports unified media keys: first_frame_image / last_frame_image / reference_images / reference_videos / reference_audios (see docs/video-api.md)
}
