...
> [service/mcp_upload_ticket.go](../service/mcp_upload_ticket.go)（上传票据签发与校验）、
...
| POST | `/v1/mcp-upload`（或 `/v1/mcp-upload/upload`） | 上传票据 或 API 令牌 | 临时图片上传（multipart，2 小时后自动删除） |
...
`/v1/mcp-upload` 支持两种认证方式（`middleware.MCPUploadAuth`）：

- **上传票据（Agent 推荐）**：MCP 工具 `request_upload_ticket` 签发的短时效 HMAC 票据，
  Agent 无需接触 API 令牌（详见第 2 节）。票据经 query `?ticket=` 或 header `X-MCP-Upload-Ticket` 传递。
- **API 令牌（人工调试）**：请求未携带票据时回退 `TokenAuth`（`Authorization: Bearer sk-xxx`）。

## 2. 临时图片上传

`POST /v1/mcp-upload`，`Content-Type: multipart/form-data`，字段 `file`。

### 2.1 Agent 推荐流程（票据认证，无需 API 令牌）

MCP Agent（模型本身）接触不到 MCP 客户端配置的 API 令牌，因此上传采用「MCP 工具签发票据 +
Agent 凭票据调 REST」的流程：

**第 1 步**：调用 MCP 工具 `request_upload_ticket`（无参数）：
...
```json
{
  ...
  "id": 1,
  "method": "tools/call",
  "params": { "name": "request_upload_ticket", "arguments": {} }
}
```

返回（TextContent，JSON 字符串）：

```json
{
  "upload_url": "https://your-server/v1/mcp-upload",
  "ticket": "mcp-upload:1000:1735889000.a1b2c3...（HMAC 签名）",
  "expires_in": 600,
  "file_field": "file",
  "max_size_bytes": 20971520,
  "allowed_types": ["image/png", "image/jpeg", "image/webp", "image/gif"],
  "image_ttl_seconds": 7200,
  "example": "curl -F file=@image.png 'https://your-server/v1/mcp-upload?ticket=mcp-upload:1000:...'",
  "next_step": "Use the returned data.id in generate_image (image_ids), ..."
}
```

**第 2 步**：执行返回的 `example` 命令（或任何 multipart POST），票据传 query `?ticket=` 或
header `X-MCP-Upload-Ticket`。票据有效期 **10 分钟**，窗口内可多次上传（多张参考图无需重复取票）。
票据无效或过期返回 401：

```json
{ "success": false, "message": "invalid or expired upload ticket: upload ticket expired" }
```

**第 3 步**：用响应中的 `data.id` 调 `generate_image` / `generate_video_from_frames` /
`generate_video_from_reference`（见下文）。

票据实现：`service.GenerateMCPUploadTicket` / `ValidateMCPUploadTicket`，格式
`mcp-upload:<userID>:<expUnix>.<HMAC-SHA256 hex>`，密钥复用站点 `CRYPTO_SECRET`
（未设置时回退 `SESSION_SECRET`；未配置固定 secret 的单实例部署重启后未使用票据作废）。
...
### 3.1 `request_upload_ticket` — 签发上传票据

无参数。返回上传票据与上传接口信息（含可照抄的 curl 示例），供 Agent 将本地图片上传到
`/v1/mcp-upload` 后以临时图片 ID 引用。详见第 2.1 节。

### 3.2 `generate_image` — 文生图 / 图生图
