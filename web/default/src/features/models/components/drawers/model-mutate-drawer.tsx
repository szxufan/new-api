...
// 侧栏仅提供跳转入口，避免与全局计费配置形成两处不一致的写入路径
// （历史实现中此处写入的倍率对非 root 管理员会静默失效）。
export const MODEL_PRICING_SETTINGS_URL =
  '/system-settings/billing/model-pricing'

// Model metadata form schema (pricing fields are managed in system settings)
      ...
      <SheetContent className='flex h-dvh w-full flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl'>
        ...
        <Form {...form}>
          ...
          >
            ...
            {/* Pricing Configuration（价格统一在「系统设置 → 计费」中管理，此处仅提供跳转入口） */}
            <div className='space-y-3'>
              ...
              <p className='text-muted-foreground text-sm'>
                {t(
                  'Model pricing (per-token ratios, per-request prices and billing expressions) is managed in Billing Settings. Saving this form only updates model metadata, not billing.'
                )}
              </p>
              <Button
                type='button'
                ...
                className='w-fit'
                onClick={() =>
                  window.open(MODEL_PRICING_SETTINGS_URL, '_blank')
                }
              >
