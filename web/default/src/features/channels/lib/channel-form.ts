...
export const channelFormSchema = z.object({
  ...
  // models 不在 schema 层强制非空：空 models 也允许提交，
  // 若 model_mapping 中存在缺失的源模型，会在 onSubmit 中弹窗引导用户补全。
  models: z.string(),
  ...
  response_detection_treat_empty_as_hit: z.boolean().optional(),
  ...
  if (channel.setting) {
    try {
      ...
      extraSettings = {
        ...
        response_detection_treat_empty_as_hit:
          parsed.response_detection?.treat_empty_as_hit || false,
      }
    } catch (error) {
