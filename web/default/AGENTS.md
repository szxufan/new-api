...
- **2026-08-28**：AI 优化提示词候选模型改为管理员指定：后端 `models` 表新增 `prompt_optimize` 字段（int 0/1 默认 0，`Update()` Select 白名单同步），`GET /api/user/models` 新增 `?prompt_optimize=true` 过滤（`filterPromptOptimizeModels` + 单测）；/models/metadata 编辑抽屉新增「可用于 AI 优化提示词」开关（`modelFormSchema` + Switch + 单测）；/image-debug 候选 = 标记 ∩ 用户可用 ∩ openai 端点。
- **2026-08-28**：视频提示词 AI 优化升级为分镜头脚本：按表单时长生成以 5 秒为单位的分镜（前端计算 `ceil(时长/5)` 写入用户消息；-1 智能时长由模型自定；≤0 按 5 秒）；视频表单输入图片以「文本 + image_url」多模态消息一并传给优化模型（`OptimizeContentPart` 类型，后端 ParseContent 原生支持）；`VIDEO_PROMPT_OPTIMIZE_SYSTEM_PROMPT` 重写；新增 6 个单测用例；i18n 六语言新增 3 键并移除废弃键 `No text models available`。
- **2026-08-28**：修复「可用于 AI 优化提示词」标记对非精确名称规则不生效：`?prompt_optimize=true` 过滤原先仅按模型名精确匹配元数据记录，前缀/后缀/包含规则（如以「前缀」标记 `qwen3.8`）下的实际模型（`qwen3.8-*`）无法命中，导致候选为空。改为 `GetPromptOptimizeModels()` 返回标记记录、`matchesPromptOptimizeModel` 按 `name_rule` 匹配（与 updatePricing 的元数据匹配方式一致）；新增 `TestGetUserModelsPromptOptimizeFilterNameRule` 覆盖前缀/后缀/包含三种规则。
...
- **2026-09-01**：渠道级独立重试次数：`channels` 表新增 `retry_times` 列（*int 默认 0；0=跟随全局 / -1=禁止该渠道重试 / N>0=该渠道重试 N 次，覆盖全局）。后端 `controller/relay.go` 新增 `resolveChannelAttempts`/`computeExtraRetryBudget`，轮次循环/fallback/亲和性锁定/检测命中四分支按渠道覆盖解析，首轮预算补偿保证无覆盖时行为不变；`SetupContextForSelectedChannel` 注入 `ContextKeyChannelRetryTimes`；`validateChannel` 校验 ≥-1。前端渠道编辑抽屉 Routing & Overrides 新增「渠道重试次数」输入（-1~50），`channel-form.ts` schema/转换同步；文档 `docs/channel-retry-times.md`；i18n 六语言新增 2 键；顺带修复 `channel-actions.ts` 存量 TS 错误（`response.time` possibly undefined）。
